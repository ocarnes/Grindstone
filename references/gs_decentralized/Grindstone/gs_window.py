...
class GrindstoneWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent, gs_path):
        ...
        for stage in self.pipelineStages:
            ...
            #parent.setBackground(0, "QCheckBox {background-color: rgb(240, 240, 240);}")
            #parent.setBackground(0, QtGui.QBrush(QtGui.QColor("red"))))
    ...
    def testFunc(self):
        ...
        #self.pipelineStages[len(self.pipelineStages) - 1].scripts[0].doCheck()

        #dynamicButton =  QtWidgets.QPushButton("New Button", self.results)
            ...
            if category.checkState(0) != Qt.Unchecked:
                ...
                for j in range(scriptCount):
                    ...
                    if script.checkState(0) != Qt.Unchecked:
                        ...
                        label = str(self.pipelineStages[i].scripts[j].doCheck())
                        dynamicButton = QtWidgets.QPushButton(label)
                        dynamicButton.clicked.connect(self.stub)
                        self.resultsLayout.addWidget(dynamicButton)
